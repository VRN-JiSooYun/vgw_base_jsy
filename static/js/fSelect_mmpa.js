        ...
        fSelect.prototype = {
            create: function() {
                ...
                if (this.settings.showButtons) {
                    search = '<div class="fs-buttons"><div><a class="fs-button fs-select-all btn btn-sm btn-info btn-c-w">' + this.settings.selectAllText + '</a></div><div><a class="fs-button fs-deselect-all btn btn-sm btn-info btn-c-w">' + this.settings.deselectAllText + '</a></div></div>';
                    //this.$wrap.find('.fs-dropdown').prepend(search);
                }
                var html = '<div class="fs-label-wrap"><div class="fs-label"></div><span class="fs-arrow"></span></div>';
                html += '<div class="fs-dropdown hidden">{search}{no-results}{button}<div class="fs-options">' + choices_html + '</div></div>';
                html = html.replace('{search}', search_html);
                html = html.replace('{no-results}', no_results_html);
                html = html.replace('{button}', search);
