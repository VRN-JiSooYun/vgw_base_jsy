    ...
    $.ajax({
        ...
        success:function(msg){
            ...
            $.each(psReqInfo, (i, elem) => {
                ...
                var mean = parseFloat(elem["mean"]).toFixed(4);
                var sd = parseFloat(elem["sd"]).toFixed(1);
                var cv = parseFloat(elem["cv"]).toFixed(1);
                var time = parseFloat(elem["time"]);
                var hematocrit = parseFloat(elem["remaining"]).toFixed(3);
                var krbc = parseFloat(elem["volume"]).toFixed(4);
                ...
                if (prevComp != curComp) {
                    ...
                    html += `
                        <thead>
                            <tr><th colspan="10" style="text-align: left; padding-top: 12px; border-left: 1px solid white; border-right: 1px solid white; ${prevComp == "" ? "border-top: 1px solid white;" : ""}">${curComp}</th></tr>
                            <tr style="background-color:black; color:white;">
                                <th>Sample Name</th><th>Analyte Peak Area (counts)</th><th>IS Peak Area (counts)</th><th>Area Ratio</th><th>hematocrit</th><th>Mean</th><th>SD</th><th>RSD</th><th>Krbc/p</th>
                            <tr>
                        </thead>
                        <tbody>
                        <tr>
                            ...
                            <td>${isPeakArea}</td>
                            <td>${areaRatio}</td>
                            <td>${prevComp != curComp ? hematocrit : ''}</td>
                            <td>${endOfSampleName == "1" ? mean : ''}</td>
                            <td>${endOfSampleName == "1" ? sd : ''}</td>
                            <td>${endOfSampleName == "1" ? cv : ''}</td>
                            <td style="color: blue;"><b>${endOfSampleName == "2" && tissue != 'control' ? krbc: ''}</b></td>
                        </tr>
                    `;
                }
                else {
