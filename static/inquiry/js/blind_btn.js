function getCommentStr(csrftoken, user_id, postElem, CommentElem, marginLeft, isBest, index) {
    ...
    if (isBest) {
        replyAndSelect = `
            ...
            ${ postElem['member_id'] == user_id
                ? `<a class="inquiry-cancel-best-comment" href="/inquiry/blind/cancel-best-comment?post=${ postElem['id'] }&comment=${ CommentElem['id'] }" style="float:left; margin-left:10px; font-size: 14px; color:silver;">채택 취소</a>`
                : ''
            }
    ...
    //================================================================================================================
        ...
        replyAndSelect = `
            ...
            ${ postElem['member_id'] == user_id
                ? `<button class="inquiry-comment-check-btn" link="/inquiry/blind/check-best-comment?post=${ postElem['id'] }" value="${ CommentElem['id'] }" style="float:left; margin-left:10px; font-size: 14px; color:silver; border: none; background-color:white;">채택 하기</button>`
                : ''
            }
            <div style="clear: both;"></div>
            <form method="POST" action="/inquiry/blind/write-reply" class="inquiry-write-reply-form ${ CommentElem['id'] }" style="display:none; margin-top:5px;">
                <input type="text" name="csrfmiddlewaretoken" value="${ csrftoken }" style="display:none;">
                <input type="radio" name="anonymous" value="true" checked>&nbsp;익명으로&nbsp;&nbsp;
                <input type="radio" name="anonymous" value="false">&nbsp;기명으로
    ...
    // comment content
    //================================================================================================================
    console.log("check_anonymity:", CommentElem['check_anonymity']);
    return `
        <div style="margin:8px; margin-left:${marginLeft}px; background-color:rgba(255, 255, 255, 0.9); border-left: rgba(255, 255, 255, 0.75) solid 8px; border-top: rgba(255, 255, 255, 0.75) solid 8px; border-right: rgba(255, 255, 255, 0.9) solid 8px">
            <div>
                <div>
                    <b>${ CommentElem['check_anonymity'] == 0 ? CommentElem['member_name'] : '익명' }</b>
                    ...
                    ${ CommentElem['member_id'] == user_id && !isBest
                        ? `<button class="inquiry-comment-delete-btn" href="/inquiry/blind/delete-comment?post=${ postElem['id'] }&comment=${ CommentElem['id'] }" style="float: right; border: solid 1px; margin-right:10px; font-size: 14px;">Delete</button>`
                ...
                <form class="inquiry-modify-comment-form ${ CommentElem['id'] }" method="POST" action="/inquiry/blind/modify-comment" style="display:none;">
                    <input type="text" name="csrfmiddlewaretoken" value="${ csrftoken }" style="display:none;">
                    <input type="text" name="inquiry-post-id" value="${ postElem['id'] }" style="display:none;">
...
function displayPostPanel(user_id, inquiry_post, categories, comments, is_p, is_r, is_v, is_d) {
    ...
    return `
        <div>
            <div>
                내용
                <a href="/inquiry/blind" style="float:right;"><button class="inquiry-question-btn" style="border: solid 1px gray;">&#9999;&nbsp;&nbsp;글쓰기</button></a>
            </div>
            <hr>
            <div class="inquiry-title">
                <div style="float:left; font-size:22px;"><b>[${ inquiry_post['check_hide'] == 0 ? '공개' : '비공개'}] ${ inquiry_post['title'] }</b></div>
                <div style="float:right;">${ inquiry_post['create_date'] }&nbsp;&nbsp;&nbsp;</div>
                <div style="float:right;">${ inquiry_post['check_hide'] == 0 ? inquiry_post['member_name'] : '익명' }&nbsp;&nbsp;&nbsp;</div>
            </div>
            <div style="clear:both;"></div>
            <input class="inquiry-title" style="width:100%; display:none;" value="${ inquiry_post['title'] }">
            <form class="inquiry-modify-post-content-form" method="POST" action="/inquiry/blind/modify-question" style="width:100%;">
                <input type="text" name="csrfmiddlewaretoken" value="${ csrftoken }" style="display:none;">
                <select name="inquiry-category" class="inquiry-category" style="display: none;">
                    <option value="open">공개</option>
                    <option value="close" ${ inquiry_post['check_hide'] == 1 ? 'selected' : ''}>비공개</option>
                </select>
                <input class="post-password" type="text" name="post-password" value="" placeholder="비밀번호 (최대 10자)" style="visibility:hidden;"/>
                <hr>
            ...
            <div style="margin:20px;">
                <form method="POST" action="/inquiry/blind/write-comment">
                    <input type="text" name="csrfmiddlewaretoken" value="${ csrftoken }" style="display:none;">
                    <div>
                        <b>댓글 달기:</b>&nbsp;&nbsp;&nbsp;&nbsp;
                        <input type="radio" name="anonymous" value="true" checked>&nbsp;익명으로&nbsp;&nbsp;
            ...
            <hr>
            <div>
                <a href="/inquiry/blind"><button class="inquiry-post-to-list-btn" style="float: left;">&#127757;&nbsp;&nbsp;목록</button></a>
                ${ inquiry_post['member_id'] == user_id ? `<button class="inquiry-post-delete-btn" href="/inquiry/blind/delete-post?post=${ inquiry_post['id'] }" style="float: right;">글 삭제 <span style="font-size: 12px;">&#10060;</span></button>` : ''}
                ${ inquiry_post['member_id'] == user_id ? `<button class="inquiry-post-modify-btn" style="float: right; margin-right:10px;">글 수정&nbsp;&#9999;</i></button>` : ''}
            </div>
        </div>
    `;
}