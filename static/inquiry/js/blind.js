...
$(document).on("click", "[href*='get-post-list']:not(.inquiry-post-table-sort)", e => {
    ...
    if (urlToRequest.includes('?') == false) urlToRequest += '?';
...
$(document).on("click", "[href*='blind']:not([href*='get-post-list'],.inquiry-cancel-best-comment,.inquiry-comment-delete-btn,.inquiry-post-delete-btn,[href*='callPopup'])", e => {
    e.preventDefault();
...
$(document).on("submit", "form[action*='blind']:not(.inquiry-search-by-keyword)", e => {
    e.preventDefault();
    console.log("clicked target:", $(e.target));

    // if ($(e.target).attr("class") == "inquiry-write-question-form" && $('.inquiry-write-question-form').find('.inquiry-category').val() == "close") {
    //     if ($('.inquiry-write-question-form').find('.post-password').val() == "") { alert("게시글 비밀번호를 입력해주세요."); return;}
    //     else if ($('.inquiry-write-question-form').find('.post-password').val().length > 10) { alert("게시글 비밀번호를 10자 내외로 입력주세요."); return;}
    // }

    // if ($(e.target).attr("class") == "inquiry-modify-post-content-form" && $('.inquiry-modify-post-content-form').find('.inquiry-category').val() == "close") {
    //     if ($('.inquiry-modify-post-content-form').find('.post-password').val() == "") { alert("게시글 비밀번호를 입력해주세요."); return;}
    //     else if ($('.inquiry-modify-post-content-form').find('.post-password').val().length > 10) { alert("게시글 비밀번호를 10자 내외로 입력주세요."); return;}
    // }
...
$(document).on('click', '.inquiry-post-modify-cancel-btn', (e) => {
    ...
    $('.post-password').css('visibility', 'hidden');
    $('.inquiry-category').val('open');
});
